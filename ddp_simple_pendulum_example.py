...
def return_l_func(Cost='Minimize Input Energy'):
    """
    Should use the current timestep, not the t + dt (or prime notation).

    Cost should be either 'Minimize Input Energy' (Default), 'Minimize time away from target angle', or 'Minimize time away from target angular velocity'. To be set upstream by linearized cost function.
    """
    if type(Cost)==str:
        assert Cost in ['Minimize Input Energy',
                    'Minimize time away from target angle',
                    'Minimize time away from target angular velocity'],\
            "Cost must be either 'Minimize Input Energy','Minimize time away from target angle', or 'Minimize time away from target angular velocity'."
    else:
        assert type(Cost)==list, "Cost must be a list of cost types."
        for el in Cost:
            assert type(el)==str, "Each element of Cost must be a string. Not " + str(type(el)) + "."
            assert el in ['Minimize Input Energy',
                        'Minimize time away from target angle',
                        'Minimize time away from target angular velocity'],\
                "Each element of Cost must be either 'Minimize Input Energy','Minimize time away from target angle', or 'Minimize time away from target angular velocity'. '" + el + "' not accepted."

    if "Minimize Input Energy" in Cost:
        result1 = lambda X,U,dt: np.matrix([[(1/2)*U**2*dt]])
    else:
        result1 = lambda X,U,dt: np.matrix([[0]])

    if "Minimize time away from target angle" in Cost:
        result2 = lambda X,U,dt: np.matrix([[k1*(1/2)*(X[0]-TargetAngle)**2*dt]])
    else:
        result2 = lambda X,U,dt: np.matrix([[0]])

    if "Minimize time away from target angular velocity" in Cost:
    ...
    result = lambda X,U,dt: result1(X,U,dt) \
                            ...
                            + result3(X,U,dt)
    return(result)
def return_lx_func(Cost='Minimize Input Energy'):
    """
    Should use the current timestep, not the t + dt (or prime notation).
    ...
    result = lambda X,U,dt: result1(X,U,dt) \
                            ...
                            + result3(X,U,dt)
    return(result)
def return_lu_func(Cost='Minimize Input Energy'):
    """
    Should use the current timestep, not the t + dt (or prime notation).
    ...
    result = lambda X,U,dt: result1(X,U,dt) \
                            ...
                            + result3(X,U,dt)
    return(result)
def return_lxu_func(Cost='Minimize Input Energy'):
    """
    Should use the current timestep, not the t + dt (or prime notation).
    ...
    result = lambda X,U,dt: result1(X,U,dt) \
                            ...
                            + result3(X,U,dt)
    return(result)
def return_lux_func(Cost='Minimize Input Energy'):
    """
    Should use the current timestep, not the t + dt (or prime notation).
    ...
    result = lambda X,U,dt: result1(X,U,dt) \
                            ...
                            + result3(X,U,dt)
    return(result)
def return_luu_func(Cost='Minimize Input Energy'):
    """
    Should use the current timestep, not the t + dt (or prime notation).
    ...
    result = lambda X,U,dt: result1(X,U,dt) \
                            ...
                            + result3(X,U,dt)
    return(result)
def return_lxx_func(Cost='Minimize Input Energy'):
    """
    Should use the current timestep, not the t + dt (or prime notation).
...
def return_quadratic_cost_function_expansion_variables(
    ...
    l_func = return_l_func(Cost=Cost)
    l = list(
            map(
    ...
    lx_func = return_lx_func(Cost=Cost)
    lx = list(
            map(
    ...
    lu_func = return_lu_func(Cost=Cost)
    lu = list(
            map(
    ...
    lux_func = return_lux_func(Cost=Cost)
    lux = list(
            map(
    ...
    # returns a list of length len(Time)-1, each element with shape (n,m), where n is the number of states and m is the number of inputs.
    lxu_func = return_lxu_func(Cost=Cost)
    lxu = list(
            map(
    ...
    luu_func = return_luu_func(Cost=Cost)
    luu = list(
            map(
    ...
    # returns a list of length len(Time)-1, each element with shape (n,n), where n is the number of states.
    lxx_func = return_lxx_func(Cost=Cost)
    lxx = list(
            map(
...
def simple_pendulum_ddp(**kwargs):
    ...
    Cost = kwargs.get("Cost","Minimize Input Energy")
    if type(Cost)==str:
        assert Cost in ['Minimize Input Energy',
                    'Minimize time away from target angle',
                    'Minimize time away from target angular velocity'],\
    ...
    if type(TerminalCost)==str:
        ...
        for el in TerminalCost:
            ...
            assert el in ['Minimize final angle from target angle',
                    'Minimize final angular velocity from target angular velocity'],\
                "Each element of TerminalCost must be either 'Minimize final angle from target angle' (Default), 'Minimize final angular velocity from target angular velocity'. '" + el + "' not accepted."
    terminal_cost_func = return_terminal_cost_func(TerminalCost=TerminalCost)
    ...
    while IterationNumber <= N_iterations:
        ...
        l,lx,lu,lux,lxu,luu,lxx = \
                return_quadratic_cost_function_expansion_variables(X,U,dt,Cost=Cost)

        # initializing the lists (to be built backwards)
          # Only concerned with reaching the target angle, dont care about velocity.
        V = [np.matrix([[t1*(1/2)*(X[0,-1] - TargetAngle)**2]])] # could add (1/2)*X[1,-1]**2
        Vx = [np.matrix([[t1*(X[0,-1]-TargetAngle)],[0]])] # could include X[1,-1] for Vx[0,0]
        Vxx = [np.matrix([[t1*1,0],[0,0]])] # could include 1 for the Vxx[0,1]

        #     # Placing a cost on the target velocity as well.
        # V = [np.matrix([[t1*(1/2)*(X[0,-1] - TargetAngle)**2
        #                     + t2*(1/2)*(X[1,-1] - TargetAngularVelocity)**2]])]
        # Vx = [np.matrix([[t1*(X[0,-1]-TargetAngle)],[(X[1,-1]-TargetAngularVelocity)]])]
        # Vxx = [np.matrix([[t1*1,0],[0,t2*1]])] # could include 1 for the Vxx[0,1]
