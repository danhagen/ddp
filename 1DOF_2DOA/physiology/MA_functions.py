...
def MA_function(Parameters,θ_PS=None):
	...
	assert str(type(Parameters))=="<class 'physiology.muscle_settings.MA_Settings'>", "Parameters are not in correct namedtuple form. Should be <class 'physiology.muscle_settings.MA_Settings'> instead of " + str(type(Parameters))
	if θ_PS is None:
		θ_PS = np.pi
	else:
...
def MA_deriv(Parameters,θ_PS=None):
	...
	Parameters = return_primary_source(Parameters)
	assert str(type(Parameters))=="<class 'physiology.muscle_settings.MA_Settings'>", "Parameters are not in correct namedtuple form."
	if θ_PS is None:
		θ_PS = np.pi
	else:
		assert type(θ_PS)==float, "θ_PS must be a float."
