...
########   for 1 DOF, 1 DOA Pendulum System    ########
#######################################################
#######################################################
##                                                   ##
...
#######################################################
#######################################################
from class_1DOF_1DOA_Torq_DDP import *
from params import *
...
FilePath = save_figures("visualizations/1DOF_1DOA_Torq/DDP/","v1.0",params,ReturnPath=True,SaveAsPDF=True)
DDP.animate_trajectory(SaveAsGif=True,FileName=FilePath+"1DOF_1DOA_Torq_DDP")

#
# class DDP_1DOF_1DOA_Torq:
...
#         Horizon - Number of timesteps into the future we wish to program. Must be an integer. Default is 300. (NOTE: Simulation ends at t = Horizon*dt)
#         """
#         assert hasattr(self,'Horizon'), "Horizon is undefined."
...
#         NumberOfIterations - Number of times to iterate the DDP. Must be an integer. Default is 100.
#         """
#         assert hasattr(self,'NumberOfIterations'), "NumberOfIterations is undefined."
...
#         dt - Discrete timestep. Must be either an int, float, float32, float64, or numpy.float. Default is 0.01. (NOTE: Simulation ends at t = Horizon*dt)
#         """
#         assert hasattr(self,'dt'), "dt is undefined."
...
#         U_o - Initial input to the system. Must be either None (meaning zero initial input to the system) or an array with shape (Horizon-1,). Default is None.
#         """
#         assert hasattr(self,'U_o'), "U_o is undefined."
...
#         X_o - Initial states of the system. Must be of shape (2,).
#         """
#         assert hasattr(self,'X_o'), "X_o is undefined."
...
#         LearningRate - rate at which the system converges to the new input. Must be either an int, float, float32, float64, or numpy.float and must be between 0 and 1. Default is 0.2.
#         """
#         assert hasattr(self,'LearningRate'), "LearningRate is undefined."
...
#         R - Running cost scalar (only one input). Must be either an int, float, float32, float64, or numpy.float. Default is 0.001.
#         """
#         assert hasattr(self,'R'), "R is undefined."
...
#         """
#         Test function that ensures ALL parameters are of the correct type and shape. You may run any individual tests by calling self.test_<var_name>() at any time.
#         """
#         self.test_Horizon()
#         self.test_NumberOfIterations()
#         self.test_dt()
#         self.test_U_o()
#         self.test_X_o()
#         self.test_p_target()
#         self.test_LearningRate()
#         self.test_Q_f()
#         self.test_R()
#
#     def F1(self,x,u):
...
#             self.X[0,i+1] = (
#                 self.X[0,i]
#                 + self.F1(self.X[:,i],self.U[i])*self.dt
#             )
#             self.X[1,i+1] = (
#                 self.X[1,i]
#                 + self.F2(self.X[:,i],self.U[i])*self.dt
#             )
#
...
#         dFx[1,0] = (self.F2(x,u)-self.F2(x-h1,u))/h
#         dFx[1,1] = (self.F2(x,u)-self.F2(x-h2,u))/h
#
...
#
#         dFu[0,0] = 0
#         dFu[1,0] = (self.F2(x,u)-self.F2(x,u-h))/h
#
#         B = np.matrix(dFu*self.dt)
#         # assert np.shape(B)==(2,1) \
#         #         and str(type(B))=="<class 'numpy.matrixlib.defmatrix.matrix'>", \
#         #     "B must be a (2,1) numpy matrix. Not " + str(type(B)) + " of shape " + str(np.shape(B)) + "."
#         return(B)
...
#             RunningCost = RunningCost + 0.5 * self.R * (self.U[j]**2) * self.dt
#
#         TerminalCost = float(
...
#
#         V = [None]*self.Horizon # Each element must be a (1,1) matrix.
...
#         Qux = [None]*self.Horizon # Each element must be a (1,2) matrix.
#         Qxu = [None]*self.Horizon # Each element must be a (2,1) matrix.
...
#                 V[j] = V[j+1] - float(0.5*Qu[j].T * Quu_inv[j] * Qu[j])
#
#             #------------------------------------------------>
#             #-------------> Find the controlls -------------->
#             #------------------------------------------------>
...
#                 dX = Phi[i]*dX + B[i]*dU
#                 U_new[i] = (self.U[i] + self.LearningRate*dU).squeeze()
#
#             self.U = U_new
#
#             #------------------------------------------------>
...
#                 self.U = (self.U[0] + self.LearningRate*dU[0])*np.ones(self.Horizon-1,)
#                 self.next_X = np.zeros((2,1))
#                 self.next_X[0,0] = self.X_o[0] + self.F1(self.X_o,self.U[:,0])*self.dt
#                 self.next_X[1,0] = self.X_o[1] + self.F2(self.X_o,self.U[:,0])*self.dt
#             else:
#                 U_new = np.zeros(self.Horizon-1,)
#                 dX = np.matrix(np.zeros((2,1)))
#                 for i in range(self.Horizon-1):
...
#         plt.suptitle("1 DOF, 1DOA (Torque) via DDP",fontsize=16)
#
#         # ax1 = plt.subplot2grid((3,2),(0,0),colspan=2)
#         ax1 = plt.subplot(322)
#         ax1.plot(Time[:-1],self.U,'r')
#         ax1.plot(
#             [Time[0],Time[-1]],
#             [0]*2,
#             'k--',
...
#             ax1.set_ylim([self.U.min() - 5,self.U.max() + 5])
#
#         ax2 = plt.subplot(323)
#         ax2.plot(
#             [Time[0],Time[-1]],
...
#
#         FileName = kwargs.get("FileName","1DOF_2DOA_Torq")
...
#                                 np.linspace(
#                                     1.05*self.X[0,0],
#                                     1.05*self.X[0,0] + (45*np.pi/180)*self.U[0]/max_tau,
#                                     20
#                                 )
#                             ),
#                         -0.75*Pendulum_Length*np.cos(
...
#
#         tau1_indicator_arrow, = ax0.plot(
#                         0.75*Pendulum_Length*np.sin(1.05*self.X[0,0] + (45*np.pi/180)*self.U[0]/max_tau)
#                         + [
#                             -k*np.sin((120*np.pi/180) - 1.05*self.X[0,0] - (45*np.pi/180)*self.U[0]/max_tau),
#                             0,
#                             -k*np.sin((60*np.pi/180) - 1.05*self.X[0,0] - (45*np.pi/180)*self.U[0]/max_tau)
#                         ],
#                         -0.75*Pendulum_Length*np.cos(1.05*self.X[0,0] + (45*np.pi/180)*self.U[0]/max_tau)
#                         + [
#                             -k*np.cos((120*np.pi/180) - 1.05*self.X[0,0] - (45*np.pi/180)*self.U[0]/max_tau),
#                             0,
#                             -k*np.cos((60*np.pi/180) - 1.05*self.X[0,0] - (45*np.pi/180)*self.U[0]/max_tau)
#                         ],
#                         Color='r',
#                         lw = 2,
#                         solid_capstyle='round'
...
#         #Input
#
#         Input1, = ax1.plot([0],[self.U[0]],color = 'r')
...
#                     np.linspace(
#                         1.05*self.X[0,i],
#                         1.05*self.X[0,i] + (45*np.pi/180)*self.U[i]/max_tau,
#                         20
#                     )
#                 )
#             )
...
#                 0.75*Pendulum_Length*np.sin(1.05*self.X[0,i] + (45*np.pi/180)*self.U[i]/max_tau)
#                 + [
#                     -k*np.sin((120*np.pi/180) - 1.05*self.X[0,i] - (45*np.pi/180)*self.U[i]/max_tau),
#                     0,
#                     -k*np.sin((60*np.pi/180) - 1.05*self.X[0,i] - (45*np.pi/180)*self.U[i]/max_tau)
#                 ]
#             )
#             tau1_indicator_arrow.set_ydata(
#                 -0.75*Pendulum_Length*np.cos(1.05*self.X[0,i] + (45*np.pi/180)*self.U[i]/max_tau)
#                 + [
#                     -k*np.cos((120*np.pi/180) - 1.05*self.X[0,i] - (45*np.pi/180)*self.U[i]/max_tau),
#                     0,
#                     -k*np.cos((60*np.pi/180) - 1.05*self.X[0,i] - (45*np.pi/180)*self.U[i]/max_tau)
#                 ]
#             )
#
...
#
#             return Pendulum,tau1_indicator,tau1_indicator_arrow,Input1,Angle,AngularVelocity,TimeStamp,
#
...
#
#             return Ground,Pendulum,tau1_indicator,tau1_indicator_arrow,Pendulum_Attachment,Pendulum_Rivet,TimeStamp,Input1,Angle,AngularVelocity,
#
#         dt = Time[1]-Time[0]
#         if dt <= 0.0001:
#             framestep=2000
...
#         if SaveAsGif==True:
#             ani.save("visualizations/"+FileName+'.gif', writer='imagemagick', fps=10)
#         plt.show()
#
